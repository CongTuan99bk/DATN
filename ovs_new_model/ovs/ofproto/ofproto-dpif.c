...
#define MAX_SIZE_WHITE_LIST (1000)
...
//swap 2 values
void  swap1(void  *v1, void  *v2, size_t  size)  {
...
// return real_client's index neu tim thay, nguoc lai tra ve 12345
int check_white_list(uint32_t src_ip, uint16_t src_tcp)
{
    for(int i=0; i <  wlist.current_number_element; i++)
        {
            ...
            // fclose(log);
            if ( (wlist.arr[i].ip == src_ip) && (wlist.arr[i].tcp == src_tcp) )
                 {return i;} 
        }
    
    return 12345; // khong tim thay trong white list
}
/*ket thuc cac ham chuc nang*/

uint8_t clear_white_list()
{
    for(int i=0; i <  wlist.current_number_element; i++)
        {
            // FILE *log = fopen("/home/fil02/Desktop/avant-guard-log/log_file.txt", "a");
...
struct flow create_flow(struct dp_packet packet_in,uint flags,bool reverse) {
    ...
    // dao nguoc src/dst
    if ( reverse == true) {
        swap1(&(new_flow.tp_src),&(new_flow.tp_dst),sizeof(ovs_be16));
        swap1(&(new_flow.nw_src),&(new_flow.nw_dst),sizeof(ovs_be32));
        swap1(&(new_flow.dl_src),&(new_flow.dl_dst),sizeof(struct eth_addr));
    }
...
struct dp_packet* packet_compose(struct dp_packet packet_in,int flags_in) {
    ...
    uint16_t winsz;
    if (ntohs(flags_in) == 2) {
        new_flow = create_flow(packet_in,18,true); // packet_in = syn => gui lai goi syn/ack cho client

        seq = cookie_hash(new_flow.nw_dst, new_flow.nw_src
                                                    ,new_flow.tp_dst,new_flow.tp_src,0,0);
        ack = seq;
        tcp_heder_len = 7;
    ...
    if (!ofproto_get_flow_restore_wait()) {
        ...
        LIST_FOR_EACH_POP (am, list_node, &ams) {
            ...
            if (flagSDOVS)
            {
                ...
                if (ntohs(tflow.tcp_flags) == 2 ) { // goi syn
                
                    //timeout for SA
                    time_t current_timeout=time(NULL);
                    if (current_timeout - lasttimeout >= 5)
                    {
                            clear_white_list();
                            lasttimeout = current_timeout;
                        //     FILE *fp;
                        //     fp= fopen("/home/iot_team/DATN/OvS-LoOP/result/clear.log","a+");
                        //   fprintf(fp,"flags = %s\n","clear roif!");
                        //   fclose(fp);
                    }
                    if (check_white_list(tflow.nw_src,tflow.tp_src) == 12345) {
                        check = false;
                        uint32_t headroom = dp_packet_headroom(buffer);
                        struct dp_packet *tpacket;
                        struct flow new_flow;

                        tpacket  = dp_packet_clone_with_headroom(buffer, headroom);

                        struct ofpbuf ofpacts; 

                        ofpbuf_init(&ofpacts, sizeof(struct ofpact_output));

                        if (ntohl(tflow.nw_src) == 167772162) //10.0.0.2
                        {
                            ofpact_put_OUTPUT(&ofpacts)->port = 2;
                        }
                        else {
                            ofpact_put_OUTPUT(&ofpacts)->port = 4;
                        }
                        
                        // tao goi syn/ack
                        tpacket = packet_compose(*tpacket,tflow.tcp_flags);
                        flow_extract(tpacket,&new_flow);
                        ofproto_dpif_execute_actions(ofproto,OVS_VERSION_MAX,&new_flow,NULL,ofpacts.data,ofpacts.size,tpacket);
                ...
                } else if (ntohs(tflow.tcp_flags) == 4 ) // goi rst 
                {
                    struct tcp_header *tcp = dp_packet_l4(buffer);
                    uint32_t syncookie = ntohl(get_16aligned_be32(&tcp->tcp_seq)) ;
                    uint32_t hash = cookie_hash(tflow.nw_src,tflow.nw_dst,tflow.tp_src,tflow.tp_dst,0,0);
                    if (hash == syncookie ) {
                        check = false;
                        wlist.arr[wlist.current_number_element].ip = tflow.nw_src;
                        wlist.arr[wlist.current_number_element].tcp = tflow.tp_src;
                ...
                else if (ntohs(tflow.tcp_flags) == 16 ) // goi ACK
                {
                    struct tcp_header *tcp = dp_packet_l4(buffer);
                    int i = check_white_list(tflow.nw_src,tflow.tp_src);
                    if (i != 12345)
                    {
                        wlist.arr[i].ip = 0;
                        wlist.arr[i].tcp = 0;
