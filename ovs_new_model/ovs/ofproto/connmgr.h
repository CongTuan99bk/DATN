...
uint16_t tcp_checksum(struct iphdr *, struct tcphdr *);
void get_ip(struct flow header);
// void get_ip_rst(char *saddr, char *daddr, int sport, int dport);
void get_mac(struct flow header);
// void get_mac_rst(uint8_t sarr[], uint8_t darr[]);
// void get_tcp_rst(int sport, int dport, int syn, int rst, int ack);
