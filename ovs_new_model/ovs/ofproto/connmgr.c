...
struct ifreq ifreq_ip; /// for each ioctl keep diffrent ifreq structure otherwise error may come in sending(sendto )
int sock_raw;
// int sock_raw_normal;
unsigned char *sendbuff;
 #define DESTMAC0	0x6e
 #define DESTMAC1	0x45
 #define DESTMAC2	0x80
 #define DESTMAC3	0x8d
 #define DESTMAC4	0x60
 #define DESTMAC5	0x39

#define T_SIZE 16777216 //4294967296 //16777216
...
void get_eth_index(struct flow header)
{
    // FILE *fp;
    int add = header.nw_src;
    struct in_addr addr = {add};
    // fp=fopen("/home/fil02/index.log","a");
    //         fprintf(fp, "syn = %s \n", inet_ntoa(addr));
    //         fclose(fp);
    if (strcmp(inet_ntoa(addr), "10.0.0.2") == 0)
    {
        memset(&ifreq_i, 0, sizeof(struct ifreq));
        strncpy(ifreq_i.ifr_name,"s1-eth2",16-1);
        //  fp=fopen("/home/fil02/index.log","a");
        //     fprintf(fp, "syn = %s \n", inet_ntoa(addr));
        //     fclose(fp);
    }
    ...
    {
        memset(&ifreq_i, 0, sizeof(struct ifreq));
        strncpy(ifreq_i.ifr_name,"s1-eth1",16-1);
    }
    //  fp=fopen("/home/fil02/index.log","a");
    //         fprintf(fp, "syn = %d \n", sock_raw);
    //         fclose(fp);
	if((ioctl(sock_raw,SIOCGIFINDEX,&ifreq_i))<0)
		printf("error in index ioctl reading : %d \n", ioctl(sock_raw,SIOCGIFINDEX,&ifreq_i));

	// printf("index=%d, %s\n",ifreq_i.ifr_ifindex, ifreq_i.ifr_name);

}

void get_mac(struct flow header)
{
    // FILE *fp;
    int add = header.nw_src;
    struct in_addr addr = {add};
    // fp=fopen("/home/fil02/mac.log","a");
    //         fprintf(fp, "syn = %s \n", inet_ntoa(addr));
    //         fclose(fp);
    if (strcmp(inet_ntoa(addr), "10.0.0.2") == 0)
    {
        memset(&ifreq_c,0,sizeof(struct ifreq));
        strncpy(ifreq_c.ifr_name,"s1-eth2",IFNAMSIZ-1);
        // fp=fopen("/home/fil02/mac.log","a");
        //     fprintf(fp, "syn = %s \n", inet_ntoa(addr));
        //     fclose(fp);
    }
         ...
         memset(&ifreq_c,0,sizeof(struct ifreq));
        strncpy(ifreq_c.ifr_name,"s1-eth1",IFNAMSIZ-1);
    }
    ...
    eth->h_source[0] = header.dl_dst.ea[0];
  	eth->h_source[1] = header.dl_dst.ea[1];
   	eth->h_source[2] = header.dl_dst.ea[2];
   	eth->h_source[3] = header.dl_dst.ea[3];
   	eth->h_source[4] = header.dl_dst.ea[4];
   	eth->h_source[5] = header.dl_dst.ea[5];

   	eth->h_dest[0]    =  header.dl_src.ea[0];
   	eth->h_dest[1]    =  header.dl_src.ea[1];
   	eth->h_dest[2]    =  header.dl_src.ea[2];
  	eth->h_dest[3]    =  header.dl_src.ea[3];
   	eth->h_dest[4]    =  header.dl_src.ea[4];
   	eth->h_dest[5]    =  header.dl_src.ea[5];
...
uint16_t csumtcp(const void *data, const int length)
{
...
uint16_t tcp_checksum(struct iphdr *snd_iph, struct tcphdr *snd_tcph)
{
	...
	return(htons(csumtcp(pseudogram, pseudogram_size)));
	//return (htons(csum(snd_tcph, sizeof(struct my_tcph)) + csum(&psh, sizeof(struct psuedo_header))));
}

void get_tcp(struct iphdr *snd_iph,uint16_t dport)
{
	struct tcphdr *tcph = (struct tcphdr *) (sendbuff + sizeof (struct iphdr) + sizeof(struct ethhdr));

    //TCP Header
	tcph->source = htons ( 8000 );
	tcph->dest = htons (dport);
	tcph->seq = htonl(0);
	tcph->ack_seq = htonl(1000);
	...
	tcph->ack=1;
	tcph->urg=0;
	tcph->window = htons (8192 );	// maximum allowed window size
	tcph->check = tcp_checksum(snd_iph, tcph); //if you set a checksum to zero, your kernel's IP stack should fill in the correct checksum during transmission
	tcph->urg_ptr = 0;
	total_len += sizeof(struct tcphdr)+ sizeof(struct ethhdr);
}
 
void get_ip(struct flow header)
{
    ...
    int value = htons(header.tp_src);
    int add = header.nw_src;
    struct in_addr addr = {add};
    int dadd = header.nw_dst;
    struct in_addr dest = {dadd};
	...
	iph->saddr = inet_addr(inet_ntoa(dest));
	iph->daddr	= inet_addr(inet_ntoa(addr)); // put destination IP address
	total_len += sizeof(struct iphdr); 
	get_tcp(iph, value);
	iph->tot_len	= htons(40);//htons(total_len - sizeof(struct ethhdr));
               ...
               const char *name, const char *local_port_name)
{
    ...
    sock_raw=socket(AF_PACKET,SOCK_RAW,IPPROTO_IP);
    // FILE *fp;
    // fp=fopen("/home/iot_team/DATN/index.log","a");
    //         fprintf(fp, "syn = %ld \n", sizeof(mgr->T));
    //         fclose(fp);
    // sock_raw_normal=socket(AF_PACKET,SOCK_RAW,IPPROTO_IP);
    mgr->ofproto = ofproto;
...
char* readline(FILE *fp)
{
    ...
    while(1)
    {
        ...
        data = realloc(cursor,new_length);
        if (!data) break;
                       ...
                       const struct ofproto_async_msg *am)
{
    ...
    //             SHA256(message,strlen(message)*sizeof(char),hash);
    ...
    //             Syn_ack = false;
    //             #ifdef DEBUG
    //             fp=fopen("/home/iot_team/DATN/close1.log","a");    
    //                 fprintf(fp,"syn-ack  =  %d", Syn_ack);
    //                 fclose(fp);
    //             #endif
    ...
    //         if (Syn_ack == true)
    //         {
    //             // fp=fopen("/home/iot_team/mac.log","a");
    //             char macStr[18]; // 2*6 + 7 + 1
    //             snprintf(macStr, sizeof(macStr), "%x:%x:%x:%x:%x:%x", 
    //             header.dl_src.ea[0], header.dl_src.ea[1],
    //             header.dl_src.ea[2], header.dl_src.ea[3], 
    //             header.dl_src.ea[4], header.dl_src.ea[5]);
    //             int port_src = htons(header.tp_src);
    //             int port_des = htons(header.tp_dst);
    //             int dst = header.nw_dst;
    //             struct in_addr addr_dst = {dst};
    //             int value = header.nw_src;
    //             struct in_addr addr = {value};
                
    //             // sock_raw=socket(AF_PACKET,SOCK_RAW,IPPROTO_IP);

    //             #ifdef DEBUG
    //             fp=fopen("/home/iot_team/DATN/socket_ret.log","a");
    //             fprintf(fp, "sock = %d \n", sock_raw);
    //             fclose(fp);
    //             #endif
    //             // if(sock_raw == -1)
    ...
    //             sendbuff=(unsigned char*)malloc(4096); // increase in case of large data.Here data is --> AA  BB  CC  DD  EE
    //             memset(sendbuff,0,4096);

    //             get_eth_index(header);  // interface number
    //             get_mac(header);
    //             get_ip(header);
    ...
    //             sadr_ll.sll_addr[0]  = header.dl_src.ea[0];
    //             sadr_ll.sll_addr[1]  = header.dl_src.ea[1];
    //             sadr_ll.sll_addr[2]  = header.dl_src.ea[2];
    //             sadr_ll.sll_addr[3]  = header.dl_src.ea[3];
    //             sadr_ll.sll_addr[4]  = header.dl_src.ea[4];
    //             sadr_ll.sll_addr[5]  = header.dl_src.ea[5];

    //             // while(1)
    //             // {
    //             send_len = sendto(sock_raw,sendbuff,sizeof(struct ethhdr) + sizeof(struct iphdr) + sizeof(struct tcphdr),0,(const struct sockaddr*)&sadr_ll,sizeof(struct sockaddr_ll));
    //                 // if(send_len<0)
    //                 // {
    //                 //     //return -1;
    ...
    //                 // }
    //             // close(sock_raw);
    //             // if (current_t - last_t >= 2 && sock_raw >= 0)
    //             // {
    //             //     close(sock_raw);
    //             //     last_t = current_t;
    //             #ifdef DEBUG
    //                 fp=fopen("/home/iot_team/DATN/close.log","a");    
    //                 fprintf(fp,"%d = ", send_len);
    //                 fclose(fp);
    //                 #endif
    //             // }
    //             // close(sock_raw);
    ...
    //     // int value = header.nw_src;
    //     // struct in_addr addr = {value};
    //     //if(header.tcp_flags == 512 && (strcmp(inet_ntoa(addr), "10.0.0.1") == 0) && Syn_ack == true)  //ban tin RST
    //     else if(header.tcp_flags == 1024)
    //     {
    //         // close(sock_raw);
    //         //get seq num
    //         const char *l7 = dp_packet_get_tcp_payload(&pkt);
    //         tcp = dp_packet_l4(&pkt);
    ...
    //         // fp=fopen("/home/iot_team/check.log","a");
    //         //     fprintf(fp, "rst = %d, %d, %d, %d, %d, %d, %d, %d, %d, %d \n", hash_value[0],
    //         //     hash_value[1], hash_value[2], hash_value[3],
    //         //     hash_value[4], hash_value[5], hash_value[6],
    //         //     hash_value[7], hash_value[8], hash_value[9]);
    //         //     fclose(fp);
    //         // fp=fopen("/home/iot_team/check.log","a");
    //         //     fprintf(fp, "syn_ack = %d \n", seq);
    //         //     fclose(fp);
    //         // char bar[1000] = "echo '1' | sudo -S python3 /home/iot_team/syn.py ";
    //         //     int systemRet = system(bar);

    //         //     if(systemRet == -1){
    //         //     }
    //         if (seq - 1000 == 0)  //check cookies(1000 la cookies cho trc cua ban tin syn-ack tu phat tu scapy)
    //         {
    //                 for(int i=0;i<10;i++)
    ...
    //     }
    //     else if (htons(header.tcp_flags) == 16)
    //     {
    //         message = malloc(STR_LENGTH * sizeof(char));
    //         for(int i=0; i<10; i++)
    //         {
