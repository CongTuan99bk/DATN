...
void abnormal_detect(struct ofproto *p)
{
    ...
    {  
      ...
      fp=fopen("/home/iot_team/DATN/OvS-LoOP/result/data_recording.log","a+");
      fprintf(fp, "%f,%f,",current_flow,current_traf);
      fclose(fp);
    
    if(current_flow!=0 && current_traf!=0){
        fp=fopen("/home/iot_team/DATN/OvS-LoOP/result/flow.log","a+");
        fprintf(fp, "%f\n",current_flow);
        fclose(fp);

        fp=fopen("/home/iot_team/DATN/OvS-LoOP/result/traffic.log","a+");
        fprintf(fp, "%f\n",current_traf);
        fclose(fp);
    }
      ...
      if(p->training==1)
     {    
          fp=fopen("/home/iot_team/DATN/OvS-LoOP/result/training.log","r");
        ...
        { 
          ...
          float temp;
          // float max_sec=0;
          fp= fopen("/home/iot_team/DATN/OvS-LoOP/result/debug.log","a+");
          for(int i=0;i<p->data_counter;i++)
          {
        ...
        //   fp=fopen("/home/iot_team/DATN/OvS-LoOP/result/time.log","a+");
        //   fprintf(fp,"%lu\n",(end.tv_sec - start.tv_sec)*1000000+end.tv_usec-start.tv_usec);
        //   fclose(fp);
          fp=fopen("/home/iot_team/DATN/OvS-LoOP/result/nor_training.log","a+");
          for(int i=0;i<p->data_counter;i++)
          {
              fprintf(fp,"%f,%f,%f,%f\n",p->Data[i]->n_flow_norm,p->Data[i]->traffic_norm,p->Data[i]->LoOP,p->loOP_threshold);
          }
          fclose(fp);
      ...
      else 
      {
         ...
         p->loOP_status = LoOP_udate(p->state,p->Data,p->data_counter);
          ...
          if (p->loOP_status > p->loOP_threshold && p->reported == false)
              {
                ...
                //   {
                  trigger_send_anomaly_detection(p);
                //   execl("/home/iot_team/DATN/timeRecord.sh", "timeRecord.sh", (char*)0);
                  int systemRet = system("date +%s%N | cut -b1-13 >> /home/iot_team/DATN/timeMSG.txt");
                   if(systemRet == -1){
            }
                    p->reported = true;
                    flagSDOVS = true;
            ...
            if (p->loOP_status <= p->loOP_threshold)
            {
                p->k_flags++;
                if(p->k_flags >= 5)
                {
          ...
          fp= fopen("/home/iot_team/DATN/OvS-LoOP/result/debug.log1","a+");
          fprintf(fp,"%f,%f,%f,%f\n",p->state->traffic_norm,p->state->n_flow_norm,p->loOP_status,p->loOP_threshold);
          fclose(fp);

        //   fp= fopen("/home/iot_team/DATN/OvS-LoOP/result/matrix.log","a+");
        //   fprintf(fp,"%f\n",p->loOP_status);
        //   fclose(fp);
          }

           fp= fopen("/home/iot_team/DATN/OvS-LoOP/result/flags.log","a+");
          fprintf(fp,"flags = %d\n",flagSDOVS);
          fclose(fp);
    ...
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    if (type == OFPTYPE_FLAG)
    {
        int systemRet = system("date +%s%N | cut -b1-13 >> /home/iot_team/DATN/timeMSG.txt");
                   if(systemRet == -1){
            }
        fp1=fopen("/home/iot_team/DATN/OvS-LoOP/result/switch.log","a+");
            fprintf(fp1, "da bat dc goi tin FLAG = %d, flag =  %d,\n",type, flagSDOVS);
            fclose(fp1);
            flagSDOVS = true;
    }
 ...
 * or with no .logouts are not evictable.) */
static bool
